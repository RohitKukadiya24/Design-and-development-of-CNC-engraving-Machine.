...
DESCRIPTION:-
The primary purpose of couplings is to join two pieces of rotating equipment’s while permitting some degree of misalignment and movement or both. By careful selection, installation and maintenance of couplings, substantial savings ca be made in reduced maintenance cost and downtime.
A rigid coupling is a unit of hardware used to join two shafts within a motor or mechanical system it may be used to connect two separate systems such as motor and generator, or to repair a connection within a single system. A rigid coupling may also be added between shaft to reduce shock and wear at the point where the shafts meet.
In our project we use ALS 40 COUPLING with sitting.
...
FUNCTION:-
To convert the signals received from the system to the drivers and ultimately drivers transfer signal to stepper motor.
DESCRIPTION:-
The USB interfaced CNC controller that was developed specifically to work with the CNC USB Motion Controller Software from www.planet-cnc.com. This Interface sports 4x 10 pin PBX-Header Connections for interfacing to our existing stepper motor drivers. It has an additional header for direct connection to our RBX-1 3 channel relay board.
This software/hardware combination can be further expanded with jog key inputs and an I2C bus for nearly unlimited additional I/O. This will allow the development of jog pendants, digital readouts, and input/output cards for a variety of purposes.
...
FUNCTION: -
It provides increments in step rather than turning smoothly
DESCRIPTION: -
...
4. Gantry :- We make gantry from aluminum plate which is less In weight and gantry is mounted on X-axis which reduces weight of machine.
5. Spindle motor :- It will be mounted on gantry with the help of bolt screw and linear guide way. It will be mounted on Z-axis. . Controller board :- It controls 5 axis of the machine and it gets power from power.
6. Controller board :- It controls 5 axis of the machine and it gets power from power supply and it will control the stepper motor by help of driver. It will be connected to PC and will run with the help of software.
7. Stepper motor driver :- Here one input is from the controller and another input is from power supply. First input will control the driver and second will run the stepper motor.
54
8. Stepper motor :- It is connected with driver and gets power from power supply. It will be of 4A and 25-40 volts. Stepper motor (or step motor) is a brushless DC electric motor that divides a full rotation into a number of equal steps. The motor's position can then be commanded to move and hold at one of these steps without any feedback sensor (an open-loop controller ), as long as the motor is carefully sized to the application.
9. Limit switch :- It will be at the end of the frame and the gantry when touches to
switch it will cut off driver power and stepper motor will stop.
10. Spindle motor :-It gets power from power supply also it holds the tool and is used for
machining purpose.
